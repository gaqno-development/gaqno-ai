...
export const usePodcastTab = () => {
  ...
  const { register, handleSubmit, control, watch, formState: { errors } } = useForm<PodcastTabFormData>({
  ...
  const apiErrorMessage = generate.isError
    ? (generate.error instanceof Error ? generate.error.message : 'Erro ao gerar áudio.')
    : null;
