...
const schema = z.object({
  instruction: z.string().min(1, 'Descreva a alteração').max(2000, 'Máximo 2000 caracteres'),
});
...
export const useEditImageTab = () => {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<EditImageTabFormData>({
  ...
  const onSubmit = useCallback(
    async (data: EditImageTabFormData) => {
      ...
      const result = await edit.mutateAsync({ file, instruction: data.instruction });
  ...
  const apiErrorMessage = edit.isError
    ? (edit.error instanceof Error ? edit.error.message : 'Erro ao editar imagem.')
    : null;
