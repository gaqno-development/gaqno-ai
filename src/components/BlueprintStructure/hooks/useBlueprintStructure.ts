...
export const SECTION_ORDER = ['introdução', 'desenvolvimento', 'clímax', 'conclusão', 'outros'];

type ChapterLike = { number?: number; chapter_number?: number; title?: string; summary?: string };
...
export const useBlueprintStructure = (
  ...
  const blueprintSections = structure?.sections as Record<string, SectionLike> | undefined;

  const groupedBySection = useMemo(() => {
    const chaptersMap = new Map(chapters.map((c) => [c.chapter_number, c]));
    const groups: Record<string, Array<{ chapter: ChapterLike; index: number; existingChapter?: unknown }>> = {};
  ...
  const handleChapterClick = useCallback(
    async (chapterNumber: number, chapterData: ChapterLike) => {
      ...
      navigate(`/ai/books/${bookId}/chapters?chapter=${result.data.id}&generate=true`);
    },
